...
AddCSLuaFile( "cl_chatderma.lua" );
include("shared.lua")
include("chat.lua")
include("dialogtree.lua")
...
timer.Simple( 2, function() SpawnNPCs()  end )
...
function LoadCharacter(ply,cmd,args)
	...
	--PNPC:SetPos( ply:GetPos()+Vector(0,0,10) )
	PNPC:SetPos( Vector(3779.000000, 1635.125000, -11984.031250) )
...
function Teleport(ply,cmd,args)
	local PlayerNPC = ply:GetNetworkedEntity("PlayerNPC")
	...
	PlayerNPC:SetPos( tr.HitPos + tr.HitNormal * 2 +Vector(0,0,60) )
	
	
Msg("DEBUG - User #" ..  ply:UserID() .. " has teleported.\n")
end
concommand.Add("teleport",Teleport)
...
function NPCGetPos(ply,cmd,args)
	PlayerNPC = ply:GetNetworkedEntity("PlayerNPC")
	local position = PlayerNPC:GetPos()
	print("POS: " .. tonumber(position.x) .. " " .. tonumber(position.y) .. " ".. tonumber(position.z))
end
concommand.Add("npcgetpos",NPCGetPos)
